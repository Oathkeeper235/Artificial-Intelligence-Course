...
def jump(discs, i, type):
    discs = list(discs)

    if type == "R1":
        if i + 1 < len(discs) and discs[i + 1] == 0:
            discs[i + 1] = discs[i]
            discs[i] = 0
    elif type == "R2":
        if i + 2 < len(discs) and discs[i + 2] == 0:
            discs[i + 2] = discs[i]
            discs[i] = 0
    elif type == "L1":
...
def check_if_equal(nextJump, discs, discs_new, i, type):
    if discs is not discs_new:
        if type == "R1":
            nextJump[f'D1: Disk {discs[i]}'] = discs_new
        elif type == "R2":
            nextJump[f'D2: Disk {discs[i]}'] = discs_new
        elif type == "L1":
            nextJump[f'L1: Disk {discs[i]}'] = discs_new
        elif type == "L2":
            nextJump[f'L2: Disk {discs[i]}'] = discs_new
...
class Discs(Problem):
    ...
    def successor(self, state):
        ...
        for i in range(len(discs)):
            if discs[i] != 0:
                # Jump 1 position to the right
                discs_new = jump(discs, i, "R1")
                check_if_equal(actions, discs, discs_new, i, "R1")

                # Jump 2 positions to the right
                discs_new = jump(discs, i, "R2")
                check_if_equal(actions, discs, discs_new, i, "R2")

                # Jump 1 position to the left
                discs_new = jump(discs, i, "L1")
                check_if_equal(actions, discs, discs_new, i, "L1")

                # Jump 2 positions to the left
                discs_new = jump(discs, i, "L2")
                check_if_equal(actions, discs, discs_new, i, "L2")
...
if __name__ == "__main__":
    ...
    Discs_Instance = Discs(discs, result)

    print(astar_search(Discs_Instance).solution())
