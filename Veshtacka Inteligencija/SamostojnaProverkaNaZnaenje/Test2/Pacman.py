...
def check_if_position_valid(x, y):
    if (x, y) in obstacles:
        return False
...
def dots_check(x, y, dots):
    dots_new = list(dots)

    if (x, y) in dots:
        dots_new.remove((x, y))
        return tuple(dots_new)

    return dots


def continue_straight(x, y, direction, dots, actions):
    if direction == "istok":
        new_x = x + 1
        new_y = y
    ...
    if check_if_position_valid(new_x, new_y):
        dots_new = dots_check(new_x, new_y, dots)
        actions["ProdolzhiPravo"] = (new_x, new_y, direction, dots_new)


def continue_back(x, y, direction, dots, actions):
    if direction == "istok":
        new_x = x - 1
        new_y = y
        new_direction = "zapad"
    ...
    if check_if_position_valid(new_x, new_y):
        dots_new = dots_check(new_x, new_y, dots)
        actions["ProdolzhiNazad"] = (new_x, new_y, new_direction, dots_new)


def turn_left(x, y, direction, dots, actions):
    if direction == "istok":
        new_x = x
        new_y = y + 1
        new_direction = "sever"
    ...
    if check_if_position_valid(new_x, new_y):
        dots_new = dots_check(new_x, new_y, dots)
        actions["SvrtiLevo"] = (new_x, new_y, new_direction, dots_new)


def turn_right(x, y, direction, dots, actions):
    if direction == "istok":
        new_x = x
        new_y = y - 1
        new_direction = "jug"
    ...
    if check_if_position_valid(new_x, new_y):
        dots_new = dots_check(new_x, new_y, dots)
        actions["SvrtiDesno"] = (new_x, new_y, new_direction, dots_new)
...
class Pacman(Problem):
    ...
    def successor(self, state):
        ...
        # Continue Straight:
        continue_straight(pacman_x, pacman_y, pacman_direction, dots_array, actions)

        # Turn Back and Continue:
        continue_back(pacman_x, pacman_y, pacman_direction, dots_array, actions)

        # Turn Left:
        turn_left(pacman_x, pacman_y, pacman_direction, dots_array, actions)

        # Turn Right:
        turn_right(pacman_x, pacman_y, pacman_direction, dots_array, actions)
    ...
    def h(node):
        ...
        pacman_x = current_state[0]
        pacman_y = current_state[1]
        remaining_dots = current_state[3]

        value = 0
        for (pacman_x, pacman_y) in remaining_dots:
            value += 1
...
if __name__ == "__main__":
    ...
    pacman = Pacman((x, y, direction, dots))

    # Using a Remaining Food Heuristic Function, also tried with Manhattan method but this gave better test results
