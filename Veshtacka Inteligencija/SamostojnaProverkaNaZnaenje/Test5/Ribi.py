...
if __name__ == '__main__':
    ...
    criterion = input()
    new_input = [el for el in input().split(" ")]
    ...
    train_x1 = list()
    for row in train_x:
        train_x1.append([row[i] for i in range(len(row)) if i != col_index])
    ...
    test_x1 = list()
    for row in test_x:
        test_x1.append([row[i] for i in range(len(row)) if i != col_index])
    ...
    for i in range(len(test_set)):
        predicted_class = classifier.predict([test_x1[i]])[0]
        true_class = test_y[i]
    ...
    type_of_fish = classifier.predict([new_input])[0]
    print(type_of_fish)

    predictions = classifier.predict_proba([new_input])[0]
    print(predictions)
